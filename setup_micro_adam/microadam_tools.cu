...
__global__ void copy_values_at_relative_indices_kernel_bf16(LL d, LL k, LL d_block_size, LL k_block_size, int16 *indices, bfloat16 *vector, bfloat16 *out) {
    /*
...
__global__ void copy_values_at_relative_indices_kernel_f32(LL d, LL k, LL d_block_size, LL k_block_size, int16 *indices, float *vector, bfloat16 *out) {
    /*
        This kernel performs the operation out = vector[indices], where `indices` contains int16 values representing the
    relative indices in each block of size d_block_size, having k_block_size (last block might be shorter).
        Dimensions:
...
// 	const LL T = blockDim.x; // number of threads
    ...
    if(ki < k_index_end) { // Tid is the index for indices
        out[ki] = __float2bfloat16(vector[indices[ki] + d_index_start]);
//         printf("Bid=%ld, Tid=%ld, d_index_start=%ld, k_index_start=%ld, k_index_end=%ld, i=%ld, pos=%ld\n",
//             Bid, Tid, d_index_start, k_index_start, k_index_end, i, i + d_index_start);
    }
}
void copy_values_at_relative_indices_cuda(LL d, LL k, LL d_block_size, LL k_block_size, torch::Tensor indices, torch::Tensor vector, torch::Tensor out) {
    ...
    switch(vector.scalar_type()) {
        case torch::ScalarType::BFloat16:
            copy_values_at_relative_indices_kernel_bf16<<<blocks, threads>>>(d,
                                                                            k,
                                                                            d_block_size,
                                                                            k_block_size,
                                                                            (int16*) indices.data_ptr(),
            ...
            break;
        case torch::ScalarType::Float:
            copy_values_at_relative_indices_kernel_f32<<<blocks, threads>>>(d,
                                                                            k,
                                                                            d_block_size,
                                                                            k_block_size,
                                                                            (int16*) indices.data_ptr(),
